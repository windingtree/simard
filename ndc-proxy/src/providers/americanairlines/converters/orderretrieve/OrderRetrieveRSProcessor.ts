...
export async function processOrderRetrievalRS(soapResponse: string): Promise<NDCOrderRetrievalResponse> {
    const results = await camaro.transform(soapResponse, template);
    const ndcOrderRetrievalResponse = PlainToNDCTransformer.transformOrderRetrieveRS(results[0]);
    assertNoErrorInResponse(ndcOrderRetrievalResponse.OrderViewRS);    // check response errors and throw error if there are problems detected
    return ndcOrderRetrievalResponse;
}
